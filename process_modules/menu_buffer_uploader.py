...
class Tbf:
    ...
    def _title_text(self):
        ...
        if "_" in app_name or app_name.islower():
            return _title_case(app_name.replace("_", " "))
        return app_name
    ...
    def _status_text(self, items, selected_index):
        if not items:
            return "No items"
        selected = _clip_text(items[selected_index], 18)
    ...
    def refresh(self, state=None):
        ...
        items = [str(item) for item in getattr(self.m_b, "menu_list", [])]
        selected_index = self._normalize_cursor(items)
        top_index = self._top_index(len(items), selected_index)
        footer_text = self._status_text(items, selected_index)
        ...
        state = str(state or "")
        self._draw_footer(footer_text, state=state)
        self._flush()
